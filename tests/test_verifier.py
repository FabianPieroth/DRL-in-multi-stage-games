...
ids_verifier, testdata_verifier = zip(
    *[
        ["signaling_contest", ["signaling_contest", 0.038]],
        ["sequential_auction", ["sequential_auction", 0.25]],
    ]
)


@pytest.mark.parametrize(
    "environment, utility_in_BNE", testdata_verifier, ids=ids_verifier
)
def test_verifier_in_bne(environment, utility_in_BNE):
    ...
    for utility_loss in utility_losses.values():
        assert abs(utility_loss - utility_in_BNE) < EPS
