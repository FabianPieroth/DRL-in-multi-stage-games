...
def run_sequential_sales_interdependent_plus_risk_experiment():
    environment = "sequential_auction"
    log_path = f"{LOG_PATH}/{environment}_interdependent_plus_cara_risk_experiment/"
        ...
        for i in range(runs):
            ...
            config = io_ut.get_config(
                overrides=[
                    ...
                    f"rl_envs.prettify_plots={prettify_plots}",
                ]
            )
...
def run_signaling_contest_risk_experiment():
    environment = "signaling_contest_cara_risk"
    log_path = f"{LOG_PATH}/{environment}_experiment/"
...
def run_signaling_contest_interdependencies_experiment():
    environment = "signaling_contest_interdependencies_longer_run"
    log_path = f"{LOG_PATH}/{environment}_experiment/"
    ...
    information_cases = ["winners_signal", "winning_bids"]
    algorithms = ["reinforce", "ppo"]
    samplers = ["affiliated_uniform", "mineral_rights_common_value"]
    gae = 0.99
...
def run_signaling_contest_asymmetries_experiment():
    environment = "signaling_contest_asymmetries"
    log_path = f"{LOG_PATH}/{environment}_experiment/"
...
def run_bertrand_competition_risk_experiment():
    ...
    for option in options:
        ...
        for i in range(runs):
            print("=============\nStart new run\n-------------")
            if algorithm == "ppo" and cara_risk_aversion > 1.5:
            ...
            config = io_ut.get_config(
                overrides=[
                    ...
                    f"rl_envs.prettify_plots={prettify_plots}",
                    f"rl_envs.cara_risk_aversion={cara_risk_aversion}",
                    f"algorithm_configs.ppo.normalize_rewards={normalize_rewards}",
                ]
            )
...
def run_bertrand_competition_interdependencies_experiment():
    environment = "bertrand_competition_interdependencies_high_discr"
    log_path = f"{LOG_PATH}/{environment}_experiment/"
