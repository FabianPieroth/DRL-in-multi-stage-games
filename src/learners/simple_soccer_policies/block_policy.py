...
class BlockPolicy(EvalPolicy):
    ...
    def compute_actions(self, states: SoccerStates):
        ...
        opponent_pos = states.objects[
            ..., 1 + self.n_players : 1 + 2 * self.n_players, :, 0
        ]  # [B, K, XY]
        opponent_vel = states.objects[
            ..., 1 + self.n_players : 1 + 2 * self.n_players, :, 1
        ]  # [B, K, XY]

        opponent_distances = (opponent_pos - ours_pos).norm(dim=-1, keepdim=True)
        projected_opponent_pos = opponent_pos + 1e-1 * opponent_vel * torch.sqrt(
            opponent_distances
        )

        target_pos = projected_opponent_pos + torch.tensor(
        ...
        action_template = torch.zeros(
            states.batch_shape + (self.n_players * 3,),
            dtype=torch.int64,
            device=self.device,
        )
