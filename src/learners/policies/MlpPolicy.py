...
class CustomActorCriticPolicy(ActorCriticPolicy):
    def __init__(
        ...
        if self.action_dependent_std:
            assert (
                not self.use_sde
            ), "`action_dependent_std` and `use_sde` are incompatible."
            self.action_dist = DiagGaussianDistributionWithVariableStd(
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        elif isinstance(self.action_dist, DiagGaussianDistributionWithVariableStd):
            self.action_net, self.log_std = self.action_dist.proba_distribution_net(
                latent_dim=latent_dim_pi, log_std_init=self.log_std_init
            )
    ...
    def _get_action_dist_from_latent(self, latent_pi: th.Tensor) -> Distribution:
        """
        Retrieve action distribution given the latent codes. Overrides SB3's
        function for action dependent std functionality.
        ...
        if isinstance(self.action_dist, DiagGaussianDistributionWithVariableStd):
